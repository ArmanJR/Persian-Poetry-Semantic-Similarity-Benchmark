"""
OpenRouter Embedding Models Benchmark for Persian Poetry Explanations

This script benchmarks various embedding models available through OpenRouter API
on their ability to identify semantic outliers in LLM-generated explanations
of Persian poetry couplets (instead of the raw poetry text).

This allows comparing whether explanations improve embedding model performance
compared to raw poetry text (exp-3).
"""
...
RESULTS_CSV_PATH = 'openrouter_results_explanations.csv'

# Explanation source configuration
# Which LLM's explanations to use for embedding
EXPLANATION_SOURCE_MODEL = 'google/gemini-2.5-flash'  # Default, can be changed
...
def extract_explanations_from_item(item: Dict, explanation_model: str) -> Optional[List[str]]:
    ...
    Args:
        item: A question item from the dataset
        explanation_model: The LLM model to use for explanations

    Returns:
        List of 4 explanation strings, or None if extraction fails
    """
...
def benchmark_model(
    ...
    """
    Benchmark a single model on the dataset using explanations.

    Args:
        client: OpenRouter client instance
        model: Embedding model ID to test
        dataset: Benchmark dataset with explanations
        explanation_model: LLM model to use for explanations
    ...
    if verbose:
        ...
        print(f"Explanation Source: {explanation_model}")
    ...
    for i, item in enumerate(dataset):
        ...
        # Extract explanations from the dataset
        explanations = extract_explanations_from_item(item, explanation_model)
        if explanations is None:
            prediction_failures += 1
            if verbose:
                print(" SKIPPED (missing explanations)")
            continue
        ...
        try:
            option_embeddings = client.get_embeddings_batch(explanations, model)
...
def main():
    ...
    print("="*80)
    print("OpenRouter Embedding Models Benchmark - EXPLANATIONS")
    print("Persian Poetry Semantic Similarity (via LLM Explanations)")
    print("="*80)
    print(f"\nUsing explanations from: {EXPLANATION_SOURCE_MODEL}")
    ...
    try:
        for idx, model in enumerate(EMBEDDING_MODELS, 1):
            ...
            try:
                accuracy, correct, total, dimension = benchmark_model(
                    client, model, dataset, EXPLANATION_SOURCE_MODEL, verbose=True
                )
                results[model] = {
                    'accuracy': accuracy,
                    'correct': correct,
